...
btnTransfer.addEventListener("click", async (e) => {
  e.preventDefault();
  const amount = +inputTransferAmount.value;
  const toUsername = inputTransferTo.value.trim();
  inputTransferAmount.value = inputTransferTo.value = "";

  if (!amount || !toUsername) return;

  try {
    const res = await fetch(
      "https://bankist-server.onrender.com/api/transfer",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    // Server returns updated sender account
